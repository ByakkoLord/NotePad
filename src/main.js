...
add_button_note.addEventListener('click', () => {
    ...
    if (text_note.value == '') {
        ...
        note.innerHTML = `<h3 class="ml-5 font-bold text-lg break-words">${text_note.value}</h3><button id="add_button" class="rotate-45 flex justify-center items-center w-9 h-9 hover:bg-slate-200 hover:shadow-xl rounded-full transition-all mr-5"><i class="fa-solid fa-plus text-slate-600 text-4xl rounded-full"></i></button>`
